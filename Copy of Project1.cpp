...
void SaveLines()
{
	HANDLE file = CreateFile("Lnz.dat", GENERIC_WRITE, NULL, NULL, OPEN_ALWAYS, NULL, NULL);
	DWORD written;
	sprintf(txt, "LNZ1");
	WriteFile(file, &txt, 4, &written, NULL);
	
	WriteFile(file, &sel, 4, &written, NULL);
	
	for(int i = 0 ; i <= sel ; i++)
	{
		WriteFile(file, &l[i], sizeof(l[i]), &written, NULL);
	}
	CloseHandle(file);

	sprintf(txt, "Lnz.dat written");
	TextOut(hdc, 0, 640+18*1, txt, strlen(txt));
}
void LoadLines()
{
	HANDLE file = CreateFile("Lnz.dat", GENERIC_READ, NULL, NULL, OPEN_ALWAYS, NULL, NULL);
	DWORD written;
	
	ReadFile(file, &txt, 4, &written, NULL);
	ReadFile(file, &sel, 4, &written, NULL);
	
	for(int i = 0 ; i <= sel ; i++)
	{
		ReadFile(file, &l[i], sizeof(l[i]), &written, NULL);
	}
	CloseHandle(file);
